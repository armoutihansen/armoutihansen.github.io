...
- **All Work figures are now in-repo.** CitiBike's figure was previously an external `embed`
  ...
  recoloured to the warm-charcoal + amber palette (amber flags the highest-risk stations).
  Every `embed` is a local `/figures/*.html` file; a test asserts no embed points at an
  external host.
