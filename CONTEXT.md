...
- `/publications/` (nav: Research): a credibility stat strip in the hero (CV figures + a
  ...
  and there are **no extracted per-paper figures**. Entries may carry a **cover**: journal
  publications use the real journal cover (`/images/journals/*`); working papers and
  research projects use a screenshot of the manuscript's **first page** (`static/papers/*`
  PDFs → `static/images/publications/*`), a document artifact in the same category as a
  journal cover, paired with a "Manuscript" link so the image is never a tease (ADR 0007).
  _Avoid_: showing **h-index / i10-index** (both single-digit — they undercut rather than
  add); re-adding **extracted per-paper figures** (a plot/diagram lifted from inside a
...
- **"Predictive Completeness of Social Preference Theories"** is shown on `/publications/`
  ...
  keep it as "Work in progress." It now carries a first-page cover and a local "Manuscript"
  link (`static/papers/predictive-completeness.pdf`, ADR 0007); the owner shipped this PDF
  knowing its abstract is still a bracketed placeholder. Leave it as-is unless the owner
  says otherwise.
